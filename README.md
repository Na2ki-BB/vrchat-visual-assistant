...
- 結果パネルを左手、右手、ヘッドセット正面へ追従させ、位置・角度・大きさを画面から調整して保存
...
3. `プレビュー`を押し、ヘッドセット内で位置と大きさを確認します。
4. 必要なら位置、角度、横幅のスライダーを動かします。表示中のプレビューへ即時反映されます。
5. 問題なければ`保存`を押します。次回起動後も同じ配置を使います。
...
OSCQuery経由のButton操作と任意のOpenAI翻訳は実機確認済みです。次の短い実機ゲートは、新しい左手追従の位置調整と保存後の再起動反映です。
