...
- **マージ後は画面上の読み順へ戻すこと。** primaryと帯の行を統合した後、各行の中立なbounding boxを使ってY座標でソートし、同じ高さの行はX座標をタイブレークにしてください。現在の`primaryの全行 → 帯だけの行`という追加順のままでは、画面中央で補完した行が結果末尾へ飛び、優先度3で帯の実行機会を増やすほど読み順が崩れます。bounding boxはカバレッジ判定のために取得するので、同じ情報を再利用してください
- **行間から段落を復元すること。** Windows OCRは行を返しますが段落・ブロックを返しません。隣接行のY座標間隔が通常の行間より十分大きい箇所へ空行を挿入し、別の看板や離れた段落を連結しないでください。固定ピクセル値ではなく、認識行の高さや通常の行間を基準に判定し、解像度変更に耐えるようにしてください
